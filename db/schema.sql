...
CREATE INDEX idx_network_contacts_brand ON network_contacts(brand_id, status, updated_at DESC);
CREATE INDEX idx_network_contacts_phone
  ON network_contacts(brand_id, phone_digits)
  WHERE phone_digits IS NOT NULL AND phone_digits <> '';
CREATE INDEX idx_network_contacts_email
  ON network_contacts(brand_id, lower(email))
  WHERE email IS NOT NULL AND email <> '';
CREATE INDEX idx_network_contacts_source ON network_contacts(brand_id, source);
CREATE TRIGGER trg_network_contacts_updated_at BEFORE UPDATE ON network_contacts
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

CREATE TABLE line_network_inbox (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  brand_id        UUID NOT NULL REFERENCES brands(id) ON DELETE CASCADE,
  line_user_id    TEXT,
  line_group_id   TEXT,
...
CREATE INDEX idx_line_network_inbox_brand ON line_network_inbox(brand_id, created_at DESC);

CREATE TABLE network_match_logs (
  id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  brand_id             UUID NOT NULL REFERENCES brands(id) ON DELETE CASCADE,

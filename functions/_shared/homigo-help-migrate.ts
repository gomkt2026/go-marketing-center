...
import { HOMIGO_HELP_DOCS, HOMIGO_HELP_ORIGINS } from './homigo-help-docs';

export async function applyHomigoHelpSeed(env: Env, actorUserId?: string | null): Promise<{
  upserted: string[];
  created: number;
  updated: number;
  origins: string[];
  ...
  return withProductHelp(env, async () => {
    ...
    const brandRows = await sql`SELECT id FROM brands WHERE slug = 'homigo' LIMIT 1`;
    const brandId = (brandRows[0] as { id?: string } | undefined)?.id;
    if (!brandId) throw new Error('找不到 Homigo 品牌');

    const origins: string[] = [];
    for (const origin of HOMIGO_HELP_ORIGINS) {
      await sql`
    ...
    for (const doc of HOMIGO_HELP_DOCS) {

...
import { WASHGO_HELP_DOCS, WASHGO_HELP_ORIGINS } from './washgo-help-docs';

export async function applyWashgoHelpSeed(env: Env, actorUserId?: string | null): Promise<{
  upserted: string[];
  created: number;
  updated: number;
  origins: string[];
  ...
  return withProductHelp(env, async () => {
    ...
    if (!brandId) throw new Error('找不到 Washgo 品牌');

    const origins: string[] = [];
    for (const origin of WASHGO_HELP_ORIGINS) {
      await sql`
    ...
    for (const doc of WASHGO_HELP_DOCS) {
